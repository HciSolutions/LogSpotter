    ...
    public class LogEventCollection : IDisposable, IList<LogEvent>, IBindingListView
    {
        ...
        private void AddCore(LogEvent[] items)
        {
            ...
            lock (this)
            {
                ...
                if (items.Length > Config.Config.Current.MaxEvents)
                {
                    skipItems = items.Length - Config.Config.Current.MaxEvents;
                    _all.Clear();

                    ApplySortAndFilter(false);
                    forceResetNotify = true;
                }
                else if (_all.Count + items.Length >= Config.Config.Current.MaxEvents + MAX_EVENT_TOLERANCE)
                {
                    _all.RemoveRange(0, (_all.Count + items.Length) - Config.Config.Current.MaxEvents);
